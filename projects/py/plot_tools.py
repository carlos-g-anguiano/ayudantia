...
def plot_pull(h, pdf, xlabel, axis, return_chi2=False, integrate=False):
    ...
    y_err = np.sqrt(h[0])
    expected_events = np.zeros_like(h[0], dtype=np.float64) #Se corrigió de np.float128
    if integrate:
...
def plot_model(data, 
    ...
    if pulls and not axis_pulls:
        ...
        chi2 = plot_pull(h, pdf, xlabel, axis_pulls, return_chi2=True)
        if print_chi2_dof:
            dof_int = bins-len(pdf.params) if not print_params else bins-len(print_params.params)
            dof_int -=1
            #print(chi2)
